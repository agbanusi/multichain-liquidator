...
async function main() {
  ...
  console.log(
    `Liquidation Bot started ${JSON.stringify({
      network,
      deployment,
      liquidatorAddress,
      useFlashbots,
  ...
  let comet = (await hre.ethers.getContractAt(
    "CometInterface",
    cometAddress
  )) as CometInterface;
  ...
  console.log("prov", hre.ethers.provider);
  ...
  let liquidatableQuery = await Sleuth.querySol<[string, string[]], [string[]]>(
    liquidatableQuerySolFixed
  );
