...
function* deriveAccounts(pk: string, n: number = 10) {
  for (let i = 0; i < n; i++) yield (BigInt(pk) + BigInt(i)).toString(16);
}
...
function setupDefaultNetworkProviders(hardhatConfig: HardhatUserConfig) {
  for (const netConfig of networkConfigs) {
    hardhatConfig.networks[netConfig.network] = {
      ...
      accounts: REMOTE_ACCOUNTS ? "remote" : [...deriveAccounts(ETH_PK)],
    };
  }
}
...
const config: HardhatUserConfig = {
  ...
  networks: {
    hardhat: {
      ...
      accounts: [...deriveAccounts(ETH_PK)].map((privateKey) => ({
        privateKey,
